...
async function main() {
  ...
  if (process.env.NODE_ENV === 'test') {
    console.warn(
      '⚠️ WARNING: NODE_ENV is set to "test". The throttler limits are bypassed in the test environment.',
    );
    console.warn(
      '⚠️ Please run this script against a development or production server.',
    );
  }
  ...
  try {
    ...
    if (!loginRes.ok)
      throw new Error(
        'Initial login failed. Make sure server is running and DB is accessible.',
      );
    ...
    for (let i = 1; i <= authLimit + 5; i++) {
      ...
      if (res.status === 429) {
        console.log(
          `✅ Auth endpoint correctly blocked request #${i + 1} with 429 Too Many Requests!`,
        );
        auth429Hit = true;
        break;
      }
    ...
    assert.strictEqual(
      auth429Hit,
      true,
      `Auth rate limit (${authLimit} req/min) was not enforced!`,
    );

    // // 3. HTTP SEARCH RATE LIMIT TEST
    // const searchLimit = parseInt(
    //   process.env.RATE_LIMIT_SEARCH_LIMIT || '30',
    //   10,
    // );
    // console.log(
    ...
    //   if (res.status === 429) {
    //     console.log(
    //       `✅ Search endpoint correctly blocked request #${i} with 429 Too Many Requests!`,
    //     );
    //     search429Hit = true;
    //     break;
    //   }
    ...
    //   search429Hit,
    //   true,
    //   `Search rate limit (${searchLimit} req/min) was not enforced!`,
    // );
    ...
    ownerSocket?.onAny((eventName, ...args) => {
      if (eventName === 'exception') {
        ...
        if (payload?.message === 'Too many requests. Please slow down.') {
          console.log(
            `✅ WebSocket correctly threw a rate limit exception! Payload:`,
            payload,
          );
          wsRateLimitHit = true;
        }
    ...
    console.log(
      `Server processed at least ${messagesProcessed} normal messages.`,
    );
    assert.strictEqual(
      wsRateLimitHit,
      true,
      `WebSocket rate limit (${wsLimit} msg/min) was not enforced!`,
    );
